...
(async () => {
  ...
  await cenario(browser, 'cafe-f23', { codigo: CODIGOS.f23, chave: 'f23' }, ger('f23', 'CAFE'), [
    ...
    ['50-atividade', async p => { await clique(p, '#bt-add-ativ'); const t = await primeiraOpcao(p, 'select[data-a="talhaoId"]'); await escolhe(p, 'select[data-a="talhaoId"]', t); await escolhe(p, 'select[data-opsel]', 'Pulverização mecanizada'); await preenche(p, 'input[data-a="pessoas"]', '3'); await clique(p, '[data-ast="concluida"]'); await preenche(p, 'textarea[data-a="receita"]', '500 ml Mirus, 1,2 L Priori Top'); await preenche(p, 'input[data-a="tanques"]', '4'); await preenche(p, 'input[data-a="vazao"]', '400'); /* v94: vazão (L/ha) no lugar de litros por tanque */ await clique(p, '[data-add-maq]'); await preenche(p, 'input[data-m="nome"]', '13'); await preenche(p, 'input[data-m="horas"]', '6'); }],
    ['55-atividade2', async p => { await clique(p, '#bt-add-ativ'); const sel = 'select[data-a="talhaoId"] >> nth=1'; const t = await p.evaluate(() => { const s = document.querySelectorAll('select[data-a="talhaoId"]')[1]; const o = [...s.options].find(x => x.value && x.value !== 'geral'); return o ? o.value : ''; }); await escolhe(p, sel, t); await escolhe(p, 'select[data-opsel]', 'Calagem / gessagem'); await abrirDetails(p); await p.click('[data-ast="continua"] >> nth=1'); await p.fill('input[data-a="falta"] >> nth=1', '3 ruas do fundo'); }],
    ['60-colheita', async p => { await clique(p, '#bt-add-col'); const t = await primeiraOpcao(p, 'select[data-cc="talhaoId"]'); await escolhe(p, 'select[data-cc="talhaoId"]', t); await clique(p, '[data-cmodo="Mecânica"]'); await preenche(p, 'input[data-cc="carretas"]', '9,5'); await preenche(p, 'input[data-cc="bags"]', '3'); await preenche(p, 'input[data-cc="bagLitros"]', '1200'); /* v94: total em bags + valor da medida; sacas saem da conta */ await clique(p, '[data-cpas="1ª passada"]'); await clique(p, '[data-dest="Terreiro"]'); await preenche(p, 'input[data-cc="cereja"]', '60'); await preenche(p, 'input[data-cc="verde"]', '25'); await preenche(p, 'input[data-cc="seco"]', '15'); }],
  ...
  await cenario(browser, 'graos-f33', { codigo: CODIGOS.f33, chave: 'f33' }, ger('f33', 'GRAOS'), [
    ...
    ['30-operacao', async p => { await clique(p, '#bt-add-ativ'); const t = await primeiraOpcao(p, 'select[data-a="talhaoId"]'); await escolhe(p, 'select[data-a="talhaoId"]', t); await abrirDetails(p); const op = await p.evaluate(() => { const s = document.querySelector('select[data-opsel]'); if (!s) return ''; const o = [...s.options].find(x => x.value === 'Dessecação de pré-plantio') || [...s.options].find(x => x.value); return o ? o.value : ''; }); /* v95: pelo nome — a lista passou a ser alfabética dentro do grupo, e 'a primeira' mudaria o roteiro */ if (op) await escolhe(p, 'select[data-opsel]', op); }],
