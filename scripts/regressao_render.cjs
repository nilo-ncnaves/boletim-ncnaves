...
(async () => {
  ...
  await cenario(browser, 'graos-f33', { codigo: CODIGOS.f33, chave: 'f33' }, ger('f33', 'GRAOS'), [
    ...
    ['30-operacao', async p => { await clique(p, '#bt-add-ativ'); const t = await primeiraOpcao(p, 'select[data-a="talhaoId"]'); await escolhe(p, 'select[data-a="talhaoId"]', t); await abrirDetails(p); const op = await p.evaluate(() => { const s = document.querySelector('select[data-opsel]'); if (!s) return ''; const o = [...s.options].find(x => x.value); return o ? o.value : ''; }); if (op) await escolhe(p, 'select[data-opsel]', op); }],
