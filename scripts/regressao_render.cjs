...
(async () => {
  ...
  await cenario(browser, 'cafe-f23', { codigo: CODIGOS.f23, chave: 'f23' }, ger('f23', 'CAFE'), [
    ...
    ['50-atividade', async p => { await clique(p, '#bt-add-ativ'); const t = await primeiraOpcao(p, 'select[data-a="talhaoId"]'); await escolhe(p, 'select[data-a="talhaoId"]', t); await escolhe(p, 'select[data-opsel]', 'Pulverização mecanizada'); await preenche(p, 'input[data-a="pessoas"]', '3'); await clique(p, '[data-ast="concluida"]'); await preenche(p, 'textarea[data-a="receita"]', '500 ml Mirus, 1,2 L Priori Top'); await preenche(p, 'input[data-a="tanques"]', '4'); await preenche(p, 'input[data-a="ltanque"]', '2000'); await clique(p, '[data-add-maq]'); await preenche(p, 'input[data-m="nome"]', '13'); await preenche(p, 'input[data-m="horas"]', '6'); }],
