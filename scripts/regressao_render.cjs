...
(async () => {
  ...
  await cenario(browser, 'cafe-f23', { codigo: CODIGOS.f23, chave: 'f23' }, ger('f23', 'CAFE'), [
    ...
    ['65-nada-fito', async p => { await clique(p, '[data-resp-nada="CAFE-FITO"]'); }],   /* v68: "Nada a registrar hoje" — cai sozinho no 70-fito */
    ...
    ['75-nada-ocor-desfaz', async p => { await clique(p, '[data-resp-nada="CAFE-OCOR"]'); await clique(p, '[data-resp-nada="CAFE-OCOR"]'); }],   /* v68: grava e desfaz no 2º toque */
  ...
  await cenario(browser, 'graos-f33', { codigo: CODIGOS.f33, chave: 'f33' }, ger('f33', 'GRAOS'), [
    ...
    ['45-nada', async p => { await clique(p, '[data-resp-nada="GRAOS-FITO_OCOR"]'); }],   /* v68: cai sozinho no 50-fito */
  ...
  await cenario(browser, 'pecuaria-f26', { codigo: CODIGOS.f26, chave: 'f26' }, ger('f26', 'PECUARIA'), [
    ...
    ['25-nada-ocor', async p => { await clique(p, '[data-resp-nada="PEC-OCOR"]'); }],   /* v68: fica gravado (a seção não ganha registro) e sobe no payload */
